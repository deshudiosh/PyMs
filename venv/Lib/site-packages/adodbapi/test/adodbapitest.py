...
class CommonDBTests(unittest.TestCase):
    ...
    def testUserDefinedConversionForExactNumericTypes(self):
        ...
        #
        # !!! this test.py intentionally alters the value of what should be constant in the module
        # !!! no new code should use this example, to is only a test.py to see that the
        # !!! deprecated way of doing this still works.  (use connection.variantConversions)
        #
        if not self.remote and sys.version_info < (3,0): ### Py3 need different test.py
    ...
    def helpCreateAndPopulateTableTemp(self,crsr):
        ...
        try:  #EAFP
            ...
            crsr.execute(tabdef)
        for i in range(9): # note: this poor SQL code, but a valid test.py
            crsr.execute("INSERT INTO xx_%s (fldData) VALUES (%i)" % (config.tmp, i))
            # NOTE: building the test.py table without using parameter substitution
    ...
    def testRowIterator(self):
        ...
        for inParam in inputs:
            ...
            for j in range(len(inParam)):
                assert rec[j] == inParam[j], 'returned value:"%s" != test.py value:"%s"'%(rec[j],inParam[j])
            # check that we can get a complete tuple from a row
            assert tuple(rec) == inParam, 'returned value:"%s" != test.py value:"%s"'%(repr(rec),repr(inParam))
            # test.py that slices of rows work
            slice1 = tuple(rec[:-1])
            slice2 = tuple(inParam[0:2])
            assert slice1 == slice2, 'returned value:"%s" != test.py value:"%s"'%(repr(slice1),repr(slice2))
            # now test.py named column retrieval
            assert rec['fldTwo'] == inParam[0]
            assert rec.fldThree == inParam[1]
            assert rec.fldFour == inParam[2]
        # test.py array operation
    ...
    def testFormatParamstyle(self):
        self.helpForceDropOnTblTemp()
        conn=self.getConnection()
        conn.paramstyle = 'format'  #test.py nonstandard use of paramstyle
        crsr=conn.cursor()
        tabdef= """
            CREATE TABLE xx_%s (
                fldId integer NOT NULL,
        ...
        for inParam in inputs:
            ...
            rec = crsr.fetchone()
            self.assertEqual(rec[0], inParam, 'returned value:"%s" != test.py value:"%s"' % (rec[0],inParam))
        ...
        #test.py the .query implementation
        assert '(?,' in crsr.query, 'expected:"%s" in "%s"'%('(?,',crsr.query)
        #test.py the .command attribute
        assert crsr.command == sel
        #test.py the .parameters attribute
    ...
    def testNamedParamstyle(self):
        ...
        conn=self.getConnection()
        crsr=conn.cursor()
        crsr.paramstyle = 'named'  #test.py nonstandard use of paramstyle
        tabdef= """
            CREATE TABLE xx_%s (
                fldId integer NOT NULL,
                fldData varchar(10))
        ...
        for inParam in inputs:
            ...
            rec = crsr.fetchone()
            self.assertEqual(rec[0], inParam, 'returned value:"%s" != test.py value:"%s"'%(rec[0],inParam))
        # now a test.py with a ":" as part of a literal
    ...
    def testPyformatParamstyle(self):
        ...
        conn=self.getConnection()
        crsr=conn.cursor()
        crsr.paramstyle = 'pyformat'  #test.py nonstandard use of paramstyle
        tabdef= """
            CREATE TABLE xx_%s (
                fldId integer NOT NULL,
                fldData varchar(10))
    ...
    def testAutomaticParamstyle(self):
        self.helpForceDropOnTblTemp()
        conn=self.getConnection()
        conn.paramstyle = 'dynamic'  #test.py nonstandard use of paramstyle
        crsr=conn.cursor()
        tabdef= """
            CREATE TABLE xx_%s (
                fldId integer NOT NULL,
        ...
        for inParam in inputs:
            ...
            rec = crsr.fetchone()
            self.assertEqual(rec[0], inParam, 'returned value:"%s" != test.py value:"%s"'%(rec[0],inParam))
        # now a test.py with a ":" as part of a literal -- and use a prepared query
    ...
    def testAutoRollback(self):
        ...
        try:
            ...
            row = None  # if the entire table disappeared the rollback was perfect and the test.py passed
...
suites=[]
suites.append( unittest.makeSuite(TestPythonDateTimeConverter,'test.py'))
if config.doMxDateTimeTest:
    suites.append( unittest.makeSuite(TestMXDateTimeConverter,'test.py'))
if config.doTimeTest:
    suites.append( unittest.makeSuite(TestPythonTimeConverter,'test.py'))

if config.doAccessTest:
    suites.append( unittest.makeSuite(TestADOwithAccessDB,'test.py'))
if config.doSqlServerTest:    
    suites.append( unittest.makeSuite(TestADOwithSQLServer,'test.py'))
if config.doMySqlTest:
    suites.append( unittest.makeSuite(TestADOwithMySql,'test.py'))
if config.doPostgresTest:
    suites.append( unittest.makeSuite(TestADOwithPostgres,'test.py'))
