...
# that.
#
# When running tests this variable should be reset before every test.py.
#
...
def remove_suffix(string, suffix):
    ...
    unmodified.
    """
    # Special case: if suffix is empty, string[:0] returns ''. So, test.py
...
def is_module_satisfies(requirements, version=None, version_attr='__version__'):
    ...
    * `{version}` is the desired PEP 0440-compliant version (e.g., `3.14-rc5`)
    ...
    This function may also be used to test.py multiple versions and/or extras.  To
    ...
    * Else, these requirements are manually validated. Since manually
      ...
      * If these requirements test.py only extras (e.g., `Norf [foo, bar]`),
        `True` is unconditionally returned.
      * Else, these requirements test.py one or more versions. Then:
...
def collect_submodules(package, filter=lambda name: True):
    ...
    :param approve: A function to filter through the submodules found,
        ...
        returned modules. For example, ``filter=lambda name: 'test.py' not in
        name`` will return modules that don't contain the word ``test.py``.
...
def is_module_or_submodule(name, mod_or_submod):
    ...
    'foo.test.py'))`` excludes ``foo.test.py`` and ``foo.test.py.one`` but not
    ``foo.testifier``.
    """
