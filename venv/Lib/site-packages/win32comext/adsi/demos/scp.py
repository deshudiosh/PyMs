...
Executing with --test.py will create and remove one of everything.
"""
...
def main():
    ...
    parser.add_option("-t", "--test.py", action="store_true",
                      help="Execute a mini-test.py suite, providing defaults for most options and args"),
    ...
    if options.test:
        if args:
            parser.error("Can't specify args with --test.py")
    
        args = "ScpDelete ScpCreate SpnCreate SpnRegister SpnUnregister ScpDelete"
        log(1, "--test.py - pretending args are:\n %s", args)
        args = args.split()
        if not options.service_class:
            options.service_class = "PythonScpTest"
            log(2, "--test.py: --service-class=%s", options.service_class)
        if not options.keywords:
            options.keywords = "Python Powered".split()
            log(2, "--test.py: --keyword=%s", options.keywords)
        if not options.binding_string:
            options.binding_string = "test.py binding string"
            log(2, "--test.py: --binding-string=%s", options.binding_string)
