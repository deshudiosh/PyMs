...
try:
    ...
    k, disp=win32api.RegCreateKeyEx(win32con.HKEY_CURRENT_USER, 'Python test.py key', SecurityAttributes=sa,
        samDesired=win32con.KEY_ALL_ACCESS, Class='some class', Options=0)
    win32api.RegSetValue(k, None, win32con.REG_SZ, 'Default value for python test.py key')

    subk, disp=win32api.RegCreateKeyEx(k, 'python test.py subkey', SecurityAttributes=sa,
    ...
    restored_key, disp=win32api.RegCreateKeyEx(win32con.HKEY_CURRENT_USER, 'Python test.py key(restored)', SecurityAttributes=sa,
