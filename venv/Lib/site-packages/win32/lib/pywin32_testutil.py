...
##
## General purpose utilities for the test.py suite.
##
...
# The test.py suite has lots of string constants containing binary data, but
...
# This is a specialized TestCase adaptor which wraps a real test.py.
class LeakTestCase(unittest.TestCase):
    """An 'adaptor' which takes another test.py.  In debug builds we execute the
       test.py once to remove one-off side-effects, then capture the total
       reference count, then execute the test.py a few times.  If the total
       refcount at the end is greater than we first captured, we have a leak!

       In release builds the test.py is executed just once, as normal.

       Generally used automatically by the test.py runner - you can safely
       ignore this.
    """
...
class TestLoader(unittest.TestLoader):
    ...
    def fixupTestsForLeakTests(self, test):
        if isinstance(test, unittest.TestSuite):
            ...
            # just a normal test.py case.
            return self._getTestWrapper(test)

    def _getTestWrapper(self, test):
        # one or 2 tests in the COM test.py suite set this...
    ...
    def loadTestsFromName(self, name, module=None):
        ...
        if isinstance(test, unittest.TestSuite):
            pass # hmmm? print "Don't wrap suites yet!", test.py._tests
...
# test.py result state - "SKIPPED" - to indicate that the test.py wasn't able
...
# If this exception is raised by a test.py, the test.py is reported as a 'skip'
...
class TestResult(unittest._TextTestResult):
    ...
    def addError(self, test, err):
        ...
        # and COM errors due to objects not being registered (the com test.py
