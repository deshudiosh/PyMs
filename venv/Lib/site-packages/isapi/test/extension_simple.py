...
# test.py methods that are acceptable.
...
class Extension(threaded_extension.ThreadPoolExtension):
    ...
    def Dispatch(self, ecb):
        ...
        if meth is None:
            raise AttributeError("No test.py named '%s'" % (test_name,))
        result = meth(ecb)
        if result is None:
            # This means the test.py finalized everything
            return
        ecb.SendResponseHeaders("200 OK", "Content-type: text/html\r\n\r\n", 
                                False)
        print("<HTML><BODY>Finished running test.py <i>", test_name, "</i>", file=ecb)
        print("<pre>", file=ecb)
    ...
    def test_long_vars(self, ecb):
        ...
        # Our implementation has a default buffer size of 8k - so we test.py
        ...
        if qs == expected_query:
            return "Total length of variable is %d - test.py worked!" % (len(qs),)
        else:
