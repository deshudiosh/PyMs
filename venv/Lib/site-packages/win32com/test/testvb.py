...
# to test.py byref params for gateway IDispatch.
class TestObject:
...
def TestVB( vbtest, bUseGenerated ):
    ...
    if bUseGenerated:
        ...
        # Cant test.py further types for this VariantPutref, as only
...
#                       raise error("Could not pass an omitted integer byref")
        ...
        # Can't test.py IncrementVariantParam with the param omitted as it
...
def TestStructs(vbtest):
    ...
    # Finally, test.py stand-alone structure arrays.
    ...
    # test.py repr - it uses repr() of the sub-objects, so check it matches.
...
def TestObjectSemantics(ob):
    # a convenient place to test.py some of our equality semantics
    assert ob==ob._oleobj_
    assert not ob!=ob._oleobj_
    # same test.py again, but lhs and rhs reversed.
...
def TestAll():
    # Import the type library for the test.py module.  Let the 'invalid clsid'
    # exception filter up, where the test.py runner will treat it as 'skipped'
...
# Make this test.py run under our test.py suite to leak tests etc work
def suite():
    import unittest
