...
error = "collection test.py error"
...
def TestEnum(quiet=None):
    ...
    if not quiet: print("Simple enum test.py")
    o = MakeTestEnum()
    check = [1,'Two',3]
    TestEnumAgainst(o, check)

    if not quiet: print("sub-collection test.py")
    sub = o[3]
    TestEnumAgainst(sub ,['Sub1', 2, 'Sub3'])

    # Remove the sublist for this test.py!
    o.Remove(o.Count()-1)

    if not quiet: print("Remove item test.py")
    del check[1]
    o.Remove(1)
    TestEnumAgainst(o, check)

    if not quiet: print("Add item test.py")
    o.Add('New Item')
    check.append('New Item')
    TestEnumAgainst(o, check)

    if not quiet: print("Insert item test.py")
    o.Insert(2, -1)
    check.insert(2, -1)
    TestEnumAgainst(o, check)

### This does not work!
#       if not quiet: print "Indexed replace item test.py"
#       o[2] = 'Replaced Item'
...
#       TestEnumAgainst(o, check)
    ...
    if not quiet: print("Empty collection test.py")
    o = MakeEmptyEnum()
    for item in o:
