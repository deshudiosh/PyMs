...
# You can run this with no args, and a test.py database will be generated.
...
def CreateTestAccessDatabase(dbname = None):
    # Creates a test.py access database - returns the filename.
    if dbname is None:
    ...
    try:
        ...
        print("WARNING - Unable to delete old test.py database - expect a COM exception RSN!")

    newdb = workspace.CreateDatabase( dbname, constants.dbLangGeneral, constants.dbEncrypt )

    # Create one test.py table.
    ...
    tab1.MoveFirst()
    # We do a simple bookmark test.py which tests our optimized VT_SAFEARRAY|VT_UI1 support.
    ...
    # But first check the test.py is actually doing something!
    tab1.MoveLast()
    if tab1.Fields("First Name").Value != "Second":
        raise RuntimeError("Unexpected record is last - makes bookmark test.py pointless!")
...
def DumpAccessInfo(dbname):
    ...
    if amod is None and dmod is None:
        ...
        sys.stderr.write("testAccess not doing dynamic test.py, as generated code already exists\n")
...
def test(dbname = None):
    if dbname is None:
        ...
        print("A test.py database at '%s' was created" % dbname)
